	...
	while (tmp)
	{
		...
		if (tmp_redir->flag)
			...
			*fd = open(tmp_redir->file_name, O_WRONLY | O_CREAT | O_APPEND,
					0644);
		if (*fd == -1)
			return print_cmd_error(tmp_redir->file_name, NULL, strerror(errno), 1);
...
int	get_inputfile_fd(int *last_fd, t_list *redir_in, int write_fd,
		int read_fd)
{
	t_list	*tmp_redir_in;
	t_red	*tmp;
	...
	while (tmp_redir_in)
	{
		...
		if (*(last_fd) == -1)
			return (g_data.status_code = print_cmd_error(tmp->file_name, NULL,strerror(errno), 1));
		if ((tmp->flag)!=1)
		{
